...
def evaluate(config):
    ...
    adversarial_frames_np_path = f"{adv_path}_{config['k']}_{config['c']}_adv_images.npy"

    adversarial_video_path = f"{adv_path}_{config['k']}_{config['c']}_adv_video.avi"
    ...
    pd_array = []
    column_names = ["video_name", "source_model", "eval_model", "k", "c", "original_caption", "target", "adversarial_caption_np", "adversarial_caption_video"]
    ...
    #         conv_model.cuda()
        row_array = [video_name, model_name, model_opt_name, config['k'], config['c'], original_caption, target_class,
                        adv_numpy_caption,adv_video_caption]
        
        pd_array.append(row_array)
        
    eval_results_df = pd.DataFrame(pd_array, columns= column_names)
    csv_save_path = f"{save_path}_{config['k']}_{config['c']}_eval_summary.csv"
