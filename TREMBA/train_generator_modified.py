...
#             net.train()
    ...
    def train():
        ...
        pen = 1e-2
        x_sobel = torch.Tensor([[1.0, 0.0, -1.0], [1.0, 0.0, -1.0], [1.0, 0.0, -1.0]]).cuda()
        ...
        for batch_idx, a in enumerate(train_loader):
            ...
            gray = transforms.Grayscale()(data).cuda()

            edge_x = torch.conv2d(gray, x_sobel.unsqueeze(0).unsqueeze(0), stride=1, padding=1)
            edge_y = torch.conv2d(gray, x_sobel.T.unsqueeze(0).unsqueeze(0), stride=1, padding=1)
            ...
            for net in nets:
                ...
                edge_distance = torch.norm(noise - edge)

                # Add penalty to edge detection version of image
                loss_g = hingeloss(logits_adv, label) + pen * edge_distance 
