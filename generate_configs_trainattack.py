...
for t in attack_target_classes:
    ...
    if train_config['target'] is not None:
        save_name = "Modified_Imagenet_{}_target_{}.pytorch".format("_".join(train_config['model_list']), target)
    else:
        save_name = "Modified_Imagenet_{}_untarget.pytorch".format("_".join(train_config['model_list']))
